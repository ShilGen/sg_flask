...
def test_login_form_csrf(client):
    ...
    response = client.post("/login", data={"username": "test"}, follow_redirects=True)
    assert (
        b"CSRF Token Missing" in response.data
    )  # Убедитесь, что проверка соответствует вашему шаблону ошибки
...
def extract_csrf_token(data):
    ...
    match = re.search(
        r'name="csrf_token" type="hidden" value="([^"]+)"', data.decode("utf-8")
    )
    if match is None:
